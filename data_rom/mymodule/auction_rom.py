...
def collection_before_auction_start(cla):
    import numpy as np
    import cv2
    from function_game import imgs_set_, click_pos_reg, click_pos_2
    from property_rom import my_property_upload
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                for i in range(10):
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        imgs_ = imgs_set_(430, 100, 530, 140, cla, img, 0.85)
                        if imgs_ is not None and imgs_ != False:
                            click_pos_2(425, 490, cla)
                        else:
                            for c in range(10):
                                full_path = "c:\\my_games\\rom\\data_rom\\imgs\\auction\\clicked.PNG"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set_(10, 100, 80, 125, cla, img, 0.9)
                                if imgs_ is not None and imgs_ != False:
                                    full_path = "c:\\my_games\\rom\\data_rom\\imgs\\auction\\buy_title.PNG"
                                    img_array = np.fromfile(full_path, np.uint8)
                                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                    imgs_ = imgs_set_(450, 105, 500, 145, cla, img, 0.8)
                                    if imgs_ is not None and imgs_ != False:
                                        break
                                    else:
                                        click_pos_2(300, 160, cla)
                                else:
