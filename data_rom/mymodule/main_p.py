...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\rom\\data_rom;./data_rom" --add-data="C:\\my_games\\rom\\mysettings;./mysettings" --name rom -i="rom_macro.ico" --add-data="rom_macro.ico;./" --icon="rom_macro.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\rom\\data_rom;./data_rom" --name rom -i="rom_macro.ico" --add-data="rom_macro.ico;./" --icon="rom_macro.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        # 던전 종류
        self.dun_group_2 = QGroupBox('일반던전(시간o)')
        dun_g2_name = QComboBox()
        # list4 = ['던전 선택', '일반_업보', '일반_지옥', '일반_죄악', '일반_저주', '특수_마족', '특수_아르카스', '파티_묘지']
        # dun_g2_list = ['던전 선택', '다크디멘젼', '레이드', '기간토마키아']
        dun_g2_list = ['일반던전 선택', '드베르그']
        ...
        # 던전 종류
        self.dun_group_3 = QGroupBox('특수던전')
        dun_g3_name = QComboBox()
        # list4 = ['던전 선택', '일반_업보', '일반_지옥', '일반_죄악', '일반_저주', '특수_마족', '특수_아르카스', '파티_묘지']
        # dun_g3_list = ['데이모스전장', '모리아기지', 'coming soon']
        dun_g3_list = ['특수던전 선택', '엘프의광산', '환영의유적', '시간의미궁', '얼음신전', '혼돈의성채', '정령의성채']
    ...
    def onActivated_dunjeon_1_add(self):
        ...
        elif onDunjeon_1 == '일반던전 선택' or onDunjeon_1 == 'none' or onDunjeon_1_level == 0 or onDunjeon_1_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 and (onDunjeon_1 != '일반던전 선택' or onDunjeon_1 != 'none'):
            print('char_', char_)
            print('dun_', dun_)
    ...
    def onActivated_dunjeon_2_add(self):
        char_ = onCharacter
        dun_ = "던전_일반_" + str(onDunjeon_2) + "_" + str(onDunjeon_2_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_2 == '일반던전 선택' or onDunjeon_2 == 'none' or onDunjeon_2_level == 0 or onDunjeon_2_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 and (onDunjeon_2 != '일반던전 선택' or onDunjeon_2 != 'none'):
            print('char_', char_)
            print('dun_', dun_)
    ...
    def onActivated_dunjeon_3_add(self):
        char_ = onCharacter
        dun_ = "던전_특수_" + str(onDunjeon_3) + "_" + str(onDunjeon_3_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_3 == '특수던전 선택' or onDunjeon_3 == 'none' or onDunjeon_3_level == 0 or onDunjeon_3_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 and (onDunjeon_2 != '특수던전 선택' or onDunjeon_2 != 'none'):
            print('char_', char_)
            print('dun_', dun_)
            ...
            while self.isCheck is True:
                ...
                if result_game == True and v_.now_cla != "none":
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        full_path = "c:\\my_games\\rom\\data_rom\\imgs\\check\\game_title_1.PNG"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
